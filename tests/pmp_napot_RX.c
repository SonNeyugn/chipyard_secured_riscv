...
// Configure PMP0 for NAPOT, R, and X permissions
void setup_pmp() {
  uint64_t pmpcfg0, pmpaddr0;
  // pmpcfg0 = PMP_NAPOT | PMP_R | PMP_X;
...
// Write should cause an exception
void u_mode()
  {
    printf("Switched to user mode\n");
    // uint32_t *x2 = (uint32_t *)0x80002010;
    ...
    uint32_t test = reg_read32(&test_base[0]); // Read from test base
    printf("Value read from test base 0: 0x%lx\n", test);
...
int main(void) {
  ...
  printf("Testing PMP NAPOT with R and X permissions\n");
  printf("Address of testbase: %p\n", &test_base);
